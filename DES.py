...
def subkeys(key):
    ...
    for i in range(len(PC1)):
        for j in range(len(PC1[0])):
            key56bit.append(key[PC1[i][j] - 1]) # permute and get rid of 8 bits
    halfn = len(key56bit) // 2
    c = [key56bit[:halfn]]
    d = [key56bit[halfn:]] # split the 56 bit key in half
    rounds = 16
    for i in range(rounds):
        if i == 0 or i == 1 or i == 8 or i == 15: # 1 circular bit shift left
            ci = c[i][1:]
            ci.append(c[i][0])
            di = d[i][1:]
            di.append(d[i][0])
        else: # 2 bit circular shit left
            ci = c[i][2:]
            ci.append(c[i][0])
            ci.append(c[i][1])
            di = d[i][2:]
        ...
        c.append(ci)
        d.append(di)
    c = c[1:] # Get rid of the extra key from c0 and d0
    d = d[1:]
    allk56 = []
    k56 = []
    for i in range(len(c)):
        cd = np.concatenate([c[i], d[i]]) # add two arrays together
        for k in range(len(PC2)):
            for j in range(len(PC2[0])):
                k56.append(cd[PC2[k][j] - 1]) # permute the key
        allk56.append(k56)
        k56 = []
    return allk56 # Return our 16 56-bit keys
